...
resolution-based lower bound. For two close hypotheses $t$ and $t+\Delta t$, a minimal
requirement for distinguishability is that the expected change in the MSD exceeds the
effective spatial resolution scale $\Delta x^2$,
\begin{equation}
...
This criterion is not information-theoretic and does not account for finite statistics
or noise. It serves only as an interpretive baseline and will be superseded by the
photon-limited bounds below.
...
Section~\ref{sec:photon_limited}. For anomalous diffusion with Gaussian propagator,
...
This result is the anomalous-diffusion analogue of
Eq.~\eqref{eq:self_consistent}.
...
Both regimes retain the characteristic $\Phi^{-1/3}$ scaling, demonstrating that the
photon-limited temporal-resolution exponent is universal across normal and anomalous
diffusion.
...
\item weak enough temporal correlations for effective Fisher-information scaling,
\item asymptotically efficient estimators.
\end{itemize}

Strongly non-Gaussian propagators (e.g., Lévy flights) generally require numerical
information-theoretic analysis and are addressed in Appendix~B.
